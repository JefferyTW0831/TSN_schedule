...
def rand_set_flows():
    flow_data = {}
    flow_quantity = random.randint(35, 40)
...
def rand_attribute(flow):
    ...
    start_time_rand = random.randint(0, 12)
    period_rand = random.randint(10, 26)
    times_rand = random.randint(5, 7)
    size_rand = random.randint(2, 3)
    deadline_rand = random.randint(8, 20)
    while deadline_rand > period_rand:
        deadline_rand = random.randint(8, 18)
