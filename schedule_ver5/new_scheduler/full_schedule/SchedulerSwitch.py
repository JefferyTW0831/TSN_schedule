...
class SchedulerSwitch:
    ...
    def run(self):
        ...
        if self.execution == 1:
            for i in range(1, 6):
                ...
                scheduler = FullSchedulerContinue(self.topology, self.execution, mode[i]["driving_mode"], mode[i]["direction"])
            ...
            #方法選擇
            scheduler = FullSchedulerContinue(self.topology, self.execution, mode[method_chosen]["driving_mode"], mode[method_chosen]["direction"])
            #資料回傳
    ...
    def print_message(self):
        ...
        print("注意full_schedule在SchedulerSwitch.py檔案裡面有固定execution = 2")
        print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
